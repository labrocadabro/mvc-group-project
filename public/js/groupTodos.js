...
addTodoForm && addTodoForm.addEventListener('submit', addTodo);
...
async function addTodo(e) {
	...
	const addTags = document.getElementById('addTodo-addTags');
	const todoItem = addTodoForm.querySelector('[name="todoItem"]').value;
	const groupId = addTodoForm.querySelector('[name="groupId"]').value;
	const params = (new URL(document.location)).searchParams;
	let tags;
	if (!filtersAddTodo || !addTags.checked) {
		tags = [];
	} else {
	...
	try {
		...
		console.log(data)
		if (!filtersAddTodo || !addTags.checked) {
			location.href = `/groups/todos/${groupId}`;
		} else {
			location.reload()
		}
	} catch (err) {
